...
A SceneCache holds a radar field. Blocking, a miss loads on the caller and
...
live loop to repaint when the data lands.
"""
...
class SceneCache:
    ...
    a miss starts one daemon worker per key and answers `empty` until the worker's
    ...
    def __init__(self, empty=None, keep=4, max_age=None, name="scene"):
        self.empty = empty
        self.keep = keep
