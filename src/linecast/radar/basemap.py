...
Data: Natural Earth (public domain; land, lakes, and places at 1:10m,
borders at 1:50m), simplified globally by
...
def _load_marshalled(path):
    ...
    try:
        with open(cached, "rb") as fh:
            # loads(read()): load(fh) pulls the file in tiny reads, ~5x slower
    ...
    def _load_built(self):
        ...
        try:
            with open(self._built_path(), "rb") as fh:
                fmt, sea, dots, classes = marshal.loads(fh.read())
