...
Dragging and zooming move continuously from the streets to the whole planet. Release while moving for a short coast; pause before releasing to leave the map exactly where it is. Grab it again to stop the coast immediately. Zoom steps ease into place, and the visible map keeps moving while new detail arrives. `w` `a` `s` `d` ease each keyboard pan to rest.
...
Zoom all the way out and either view becomes a globe. Drag to rotate it, or press `r` to set it spinning. `S` shades it into the daylight of this moment, with the terminator creeping and night lights from a bundled NASA Black Marble 2016 map, and `c` lays the current cloud cover over it from live satellite imagery. `--view now` opens straight to the full picture. At close zooms the lights fade out smoothly as the terrain becomes more detailed.
