...
The moon's traditional calendar follows the language: Chinese with `--lang zh`, Japanese with `ja`, Korean with `ko`, Thai with `th`, none otherwise. Pin one — the Pacific calendars, the Islamic calendar, and the Old Farmer's Almanac included — or none, for every run:
