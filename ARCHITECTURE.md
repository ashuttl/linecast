...
So `maps.py` draws, `_maps_live.py` runs, `_maps_views.py` fetches and caches, `_maps_paint.py` holds the inks, `_maps_streets.py` and `_maps_labels.py` build the street register, `_maps_ui.py` is the search and directions panels, and `_maps_style.py` is the cartography as plain data. Radar is the same: `_radar_live.py`, `_radar_frames.py` (the frame cache and prefetcher), `_radar_render.py` (the compositor), `_radar_basemap.py`, `_radar_layers.py`, `_radar_warnings.py`, and `_radar_ui.py`. Weather's rendering is in `_weather_hourly.py`, `_weather_daily.py`, `_weather_sections.py`, and `_weather_alerts.py`; tides' in `_tides_render.py`.
...
The sky is `sky.py` for the frame and `_sky_live.py` for the camera and the keys: a ticker that runs the moon view's drag-and-settle, and also a coast, a zoom, and the clock. `_planets.py` says where the planets are. `_sky_catalogue.py` holds the bundled stars, their names, the constellation figures, and the Milky Way raster; the moon's star field reads the same stars. `_sky_json.py` is the sky's `--json`. `_sky_search.py` is the `/` panel: everything searchable is built once from the catalogue and matched in memory, and a thing below the horizon gets its next rising by stepping the ephemeris. `_sky_asterisms.py` is the table of asterisms the search also offers, each a list of star designations placed from the catalogue.

The sky cultures live in the same catalogue file: `culture()` prepares one from `data/cultures.json.gz`, `figures_for` and `names_for` give its constellations and star names for a display language, and `resolve_culture` picks one from the flag, the `linecast culture` setting (`culture_cmd.py`, saved by `_config.py`), or the language, as `_lunisolar.resolve_calendar` does for the moon.

`scripts/build_sky_catalogue.py` builds the bright-star catalogue. `scripts/build_deep_stars.py` separately builds the HYG faint-star supplement into `data/stars-deep.bin.gz`; `_sky_deep.py` keeps its records packed, with a bounded cache of ten-degree sky regions decoded on demand. Negative render indices address that supplement without changing the Yale name and culture indices. Magnification below 60° gradually raises the rendering limit at night; the JSON naked-eye summary is unchanged. `scripts/build_sky_objects.py` builds the 107 Messier objects; `_sky_objects.py` paints them behind the stars as glows sized to their angular extent, and supplies names, hover areas, and search targets. Sources and the supplement's CC BY-SA license are in `data/STARS.md`; CULTURES.md has the credits the cultures' licenses require.
...
- `_vtiles.py` — OpenFreeMap vector tiles, with the OpenStreetMap US Tileservice as a fallback, decoded by `_mvt.py`; `_elevation.py` — the AWS terrain tiles; `_builtup.py` — the built-up raster; `_maps_search.py` — Photon and Nominatim; `_maps_route.py` — OSRM; `_globe_now.py` — the cloud mosaic.
