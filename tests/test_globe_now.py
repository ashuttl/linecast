...
class TestDaylight:
    def test_noon_night_and_space(self):
        ...
        (day,) = globe_now.daylight(lls, sun)
        assert day[0] == 1.0
        assert day[1] == 0.0
    ...
    def test_the_terminator_is_a_band(self):
        ...
        (day,) = globe_now.daylight([[(0.0, 90.0)]], sun)
        assert 0.0 < day[0] < 1.0


class TestFlatLls:
    def test_corners_read_the_bbox(self):
        lls = globe_now.flat_lls((-10.0, 40.0, 10.0, 50.0), 4, 2)
...
class TestClouds:
    def test_uniform_alpha_samples_uniformly(self):
        ...
        (row,) = globe_now.clouds([[(45.0, -70.0), None]], canvas)
...
class TestCityLights:
    def test_globe_lights_land_on_the_near_side(self):
        lights = globe_now.city_lights_globe(20.0, -30.0, 125.0, 80, 44)
        assert lights
    ...
    def test_same_view_is_served_from_the_memo(self, monkeypatch):
        monkeypatch.setattr(globe_now, "_lights_cache",
                            Memo(keep=globe_now._LIGHTS_KEEP))
        first = globe_now.city_lights_globe(20.0, -30.0, 125.0, 80, 44)
        assert globe_now.city_lights_globe(20.0, -30.0, 125.0, 80, 44) \
            is first
        for lon0 in (-31.0, -32.0, -33.0, -34.0):
            globe_now.city_lights_globe(20.0, lon0, 125.0, 80, 44)
        assert len(globe_now._lights_cache) == globe_now._LIGHTS_KEEP


class TestApply:
    def test_day_leaves_the_ground_alone(self):
        buf = [[(90, 110, 60)]]
        globe_now.apply(buf, [[1.0]], None, {})
        assert buf == [[(90, 110, 60)]]

    def test_night_dims_toward_blue(self):
        buf = [[(90, 110, 60)]]
        globe_now.apply(buf, [[0.0]], None, {})
        r, g, b = buf[0][0]
    ...
    def test_night_clouds_stay_faintly_visible(self):
        clear = [[(90, 110, 60)]]
        cloudy = [[(90, 110, 60)]]
        globe_now.apply(clear, [[0.0]], [[0.0]], {})
        globe_now.apply(cloudy, [[0.0]], [[1.0]], {})
        assert sum(cloudy[0][0]) > sum(clear[0][0])

    def test_city_burns_through_the_night(self):
        buf = [[(20, 25, 35)]]
        globe_now.apply(buf, [[0.0]], None, {(0, 0): 1.0})
        r, g, b = buf[0][0]
    ...
    def test_a_higher_night_floor_keeps_more_of_the_ground(self):
        ...
        default = [[(90, 110, 60)]]
        street = [[(90, 110, 60)]]
        globe_now.apply(default, [[0.0]], None, {})
        globe_now.apply(street, [[0.0]], None, {},
    ...
    def test_daylit_city_stays_dark(self):
        buf = [[(90, 110, 60)]]
        globe_now.apply(buf, [[1.0]], None, {(0, 0): 1.0})
        assert buf == [[(90, 110, 60)]]

    def test_none_pixels_survive(self):
        buf = [[None, (90, 110, 60)]]
        globe_now.apply(buf, [[0.0, 0.0]], [[0.5, 0.5]], {(0, 0): 1.0})
...
class TestRefresh:
    ...
    def test_new_frame_stitches_and_repeat_is_free(self, monkeypatch):
        ...
        assert globe_now.refresh(130.0, 208) is False
...
class TestPolarCap:
    ...
    def test_no_measure_leaves_the_mosaic_alone(self, monkeypatch):
        monkeypatch.setitem(globe_now._cloud, "cover", None)
        canvas = (bytes(4 * 4 * 4), 4, 4, 0, 0, 4)
        out = globe_now.clouds([[(85.0, 0.0)]], canvas)
        assert out[0][0] == 0.0
    ...
    def test_the_deck_is_billowed_not_flat(self, monkeypatch):
        ...
        monkeypatch.setitem(globe_now._cloud, "cover", self._cover())
        monkeypatch.setitem(globe_now._cloud, "white", 0.6)
        canvas = (bytes(4 * 4 * 4), 4, 4, 0, 0, 4)
        lls = [[(80.0, -180.0 + k * 1.7) for k in range(200)]]
        (row,) = globe_now.clouds(lls, canvas)
    ...
    def test_a_clear_ring_keeps_its_cap_clear(self, monkeypatch):
        ...
        monkeypatch.setitem(globe_now._cloud, "cover",
    ...
    def test_the_cap_continues_the_ring_sector_by_sector(self, monkeypatch):
        ...
        (cl,) = globe_now.clouds(cloudy, canvas)
        (cr,) = globe_now.clouds(clear, canvas)
        assert sum(cl) / len(cl) > 0.3
        assert max(cr) == 0.0

    def test_noise_is_stable_and_bounded(self):
        tab = globe_now._noise_grid()
        assert len(tab) == globe_now._NOISE_ROWS * globe_now._NOISE_COLS
        assert min(tab) >= 0.0 and max(tab) <= 1.0
        assert globe_now._noise_grid() is tab

    def test_the_sphere_noise_has_no_antimeridian_seam(self):
        assert abs(globe_now._fbm(80.0, -180.0)
                   - globe_now._fbm(80.0, 180.0)) < 1e-6
    ...
    def test_the_pole_wears_billows_not_a_pinch(self):
        ...
        vals = [globe_now._fbm(89.9, -180.0 + k * 10.0)
                for k in range(36)]
...
class TestInkDusk:
    def test_ink_fades_by_the_fills_night_factor(self):
        ...
        night = (0.5, 0.5, 0.5)
        dusk = globe_now.ink_dusk(lls, sun, night, 2, 2)
...
class TestDaylightIsUnchanged:
    ...
    def test_the_flat_grid_matches_the_long_way(self):
        lls = globe_now.flat_lls((-40.0, -20.0, 60.0, 55.0), 60, 40)
        for sun in self.SUNS:
            self._same(lls, sun)
