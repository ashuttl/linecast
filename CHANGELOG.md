...
- Moon: In Thai the panel follows the Thai lunar calendar: the waxing or waning day in Thai numerals (แรม ๔ ค่ำ เดือน ๙), the year's animal, the next วันพระ, and the coming festival, มาฆบูชา through ลอยกระทง. The dates come from the traditional Suriyayart arithmetic and are checked against the official holy days of 2023–2026.
- Moon: The calendar follows the language by default: `--lang zh` shows the Chinese one, `ja` the Japanese, `ko` the Korean, and `th` the Thai. `--calendar chinese`, `japanese`, `korean`, or `thai` shows one of them in any other language, with the customary English names ("End of Heat · White Dew Sep 7", "Mid-Autumn Festival Sep 25"); `--calendar none` turns it off. `linecast calendar` saves the choice the way units and icons are saved.
...
- Moon: `--calendar samoan` and `--calendar chamorro` follow the Council's American Samoa and Guam lunar calendars: thirty named nights, Masina Fou through Masina Maunā and Sinahen Håcha through Sinahi, counted from the first evening the crescent can be seen over Pago Pago or Hagåtña. `--calendar refaluwasch` shows the CNMI calendar, which sets the Refaluwasch name beside the CHamoru one on the eleven nights that tradition names.
- Moon: `--calendar islamic` follows the Umm al-Qura calendar: the Hijri date beside the phase (23 Ramadan 1447 AH), turning at your own sunset, the coming month, and the next observance from Islamic New Year through Eid al-Adha, with "begins at sunset" on the day before. The month grid sets the Hijri months in its title and each day's Hijri date in the corner of its cell, and marks the month starts and observances. The month starts are computed by Saudi Arabia's civil rule from the ephemeris and checked against the published calendar for 1423 through 1500 AH; a country's announced dates may differ by a day.
- Moon: `--calendar hebrew` follows the Hebrew calendar: the date beside the phase (20 Elul 5786), turning at your own sunset, the coming month, and the next holiday from Rosh Hashanah through Tisha B'Av, with "begins at sunset" on the day before. The month grid sets the Hebrew months in its title (Elul 5786 – Tishrei 5787) and each day's Hebrew date in the corner of its cell, names each holiday's days, and marks the month starts; the hover chip notes Rosh Chodesh. `--json` carries the date in Hebrew letters too (כ׳ אלול תשפ״ו). Every month of 5780 through 5790 is checked against Hebcal; the holidays follow the place: a location in Israel keeps one day of Yom Tov, anywhere else two.
