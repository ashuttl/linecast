...
- Sunshine: A year view (`--year`) draws a column of sky for each day of the year, sunrise and sunset appearing as the edge between night and day. Its colors follow the Apple Watch Solar Dial: blue sky, navy night, a soft band of sunrise between. A marker shows the current day and time; pointing at a day shows its sunrise, sunset, and day length. In live mode, `y` switches between the day and the year.
