...
- Sunshine: A year view (`--year`) draws a column of sky for each day of the year, sunrise and sunset appearing as the edge between night and day. Its colors follow the Apple Watch Solar Dial: blue sky, navy night, a soft band of sunrise between. A marker shows the current day and time; pointing at a day shows its sunrise, sunset, and day length, and the time under the pointer with the sky there — daylight, civil, nautical or astronomical twilight, night — or the moment itself when it is near sunrise, sunset or solar noon. In live mode, `y` switches between the day and the year.
